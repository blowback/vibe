; ============================================================
; Module: test/cases/welcome_dismissed-on-first-key-esc.asm
; Purpose: Story 4.3 AC1 — Esc variant of the dismissal-hook
...
;          0x1B (Esc / ASCII ESC) and asserts the identical
...
;              (was 'i'/0x69, now 0x1B)
;            - the subtest-6 CP comparison (was CP 'i', now CP 0x1B)
;
...
;            (the Esc byte 0x1B was not preserved)
; ============================================================
...
;; ----- Test body -----
    ...
    ;; arrives in A (typically from input_get_key); for the Esc
    ;; variant we pre-load A with 0x1B and verify it survives the
    ;; PUSH AF / POP AF brackets. The hook itself is mode-agnostic
    ;; and key-agnostic — it only cares about welcome_active.
    LD      A, 0x1B                       ; simulated keystroke: Esc / ASCII ESC
    PUSH    AF
    LD      A, (welcome_active)
    OR      A
    JR      Z, .no_welcome
