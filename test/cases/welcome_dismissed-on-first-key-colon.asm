; ============================================================
; Module: test/cases/welcome_dismissed-on-first-key-colon.asm
; Purpose: Story 4.3 AC1 — colon variant of the dismissal-hook
...
;          ':' (0x3A) and asserts the identical post-state
;          contract: the dismissal hook is mode-agnostic AND
;          key-agnostic for the welcome_active clear +
;          render_mark_all_dirty call, varying only in the
;          key-preservation byte (subtest 6). (Note: ':' is the
;          ex-command-line trigger AFTER the hook returns; the
;          hook itself fires BEFORE per-mode dispatch.)
;
...
;              (was 'i'/0x69, now ':'/0x3A)
;            - the subtest-6 CP comparison (was CP 'i', now CP ':')
;
...
;            (the colon byte 0x3A was not preserved)
; ============================================================
...
;; ----- Test body -----
    ...
    ;; arrives in A (typically from input_get_key); for the colon
    ;; variant we pre-load A with ':' (0x3A) and verify it survives
    ;; the PUSH AF / POP AF brackets. The hook itself is
    ;; mode-agnostic and key-agnostic — it only cares about
    ;; welcome_active.
    LD      A, ':'                        ; simulated keystroke: colon / 0x3A
    PUSH    AF
    LD      A, (welcome_active)
    OR      A
    JR      Z, .no_welcome
...
.ok_shadow_unchanged:
    ...
    CP      0x3A                          ; colon byte preserved? (numeric for cross-replica consistency)
