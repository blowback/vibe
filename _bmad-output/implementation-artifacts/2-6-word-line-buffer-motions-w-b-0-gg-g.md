...
| 2026-05-15 | **Story 2.6 → done — code review applied 1 patch:** (P1, CONTRACT) `is_word_char` fall-through replaced `OR 1` (mutates A on the A > 'z' path, violating the helper's own "A preserved on every path" contract) with `OR A` (preserves A; Z=0 because A >= 0x7B is nonzero). −1 B. Post-patch SHA `1d9888b0ccb3778adc80bade28eb78be6c4ab2c579854b0df014e9b57d56e41e`, byte-identical second rebuild (NFR18). Size **4375 B / ~85% of new 5120 B ceiling** / 745 B headroom (was 4376 B pre-patch). 81 pass / 1 deliberate fail (unchanged — the patch doesn't affect observable test behaviour; the trash-A condition was unreachable in practice because no caller followed up with a register-sensitive op on byte > 'z' inputs). 5 items deferred to `deferred-work.md`: motion_dollar EOF-sentinel no-op on no-LF files; motion_dollar lands on CR in CRLF lines; parser_motion-prefix-gg.asm Subtest 3 cursor-coverage gap; vacuous count_accumulator==0 assertions in no-count motion tests; motion_find_line_n contract documentation gap for the 0xFFFF "walk-to-end" sentinel. ~15 dismissed as noise / verified-safe (motion_w/b zero-count guard already present via motion_apply_count's max(1, count); motion_dollar cursor-on-LF mid-buffer unreachable by NORMAL-mode invariants; Subtest 3 checks count not status_dirty so passes correctly; assorted DOC/STYLE nits). Acceptance Auditor: **16/16 ACs MET** (AC13 by hardware-UAT attestation). |
...
Code review pass 2026-05-15: 0 decision-needed / 1 patch / 5 deferred / ~15 dismissed. Acceptance Auditor verdict: **16/16 ACs MET** (AC13 by hardware-UAT attestation). Blind Hunter + Edge Case Hunter surfaced one real contract violation (P1) and five edge cases worth tracking but not blocking.

- [x] [Review][Patch] is_word_char `OR 1` fall-through mutates A, violating its own "A preserved on every path" contract; replaced with `OR A` for correct NZ-with-A-preserved (−1 byte). Comment block tightened. Build SHA `1d9888b0ccb3778adc80bade28eb78be6c4ab2c579854b0df014e9b57d56e41e`, byte-identical second build (NFR18). Size 4375 B / ~85% of 5120 B ceiling. 81 pass / 1 deliberate fail (unchanged from pre-patch). [src/motions.asm:698]

- [x] [Review][Defer] motion_dollar is a silent no-op when cursor sits at the EOF sentinel (cursor == file_length) on a no-trailing-LF file; reachable after `w` clamps at EOF on e.g. `"hello"` then user presses `$`. Spec AC5 wants cursor = file_length - 1 in this case. Hardware UAT didn't exercise it. [src/motions.asm:922]

- [x] [Review][Defer] motion_dollar lands cursor on the CR byte in CRLF files (e.g. `"ab\r\n"` → cursor=2 on the CR). Story-2.5 added a render-side CR filter so CR is visually masked, but cursor position is "wrong" from user perspective. Hardware UAT used the LF-only `vibe.asm` fixture and didn't catch it. [src/motions.asm:922]

- [x] [Review][Defer] parser_motion-prefix-gg.asm Subtest 3 still verifies only `count_accumulator == 0` post-doubled-g — doesn't assert the new motion_gg-with-count cursor landing (would be offset 6 on the 2-line fixture). Tests pass spuriously; coverage gap rather than bug. [test/cases/parser_motion-prefix-gg.asm:139]

- [x] [Review][Defer] No-count motion tests (motions_G-no-count, motions_b-*, motions_w-*, motions_dollar-*, motions_0-*) pre-zero `count_accumulator` then assert it's still 0; sentinel 0x81 ("count_accumulator not cleared") adds zero coverage. Could add a non-zero-pre-seed variant. [test/cases/motions_*]

- [x] [Review][Defer] motion_find_line_n's contract reads `In: DE = target line number (>= 1)`; motion_G no-count branch passes `DE = 0xFFFF` as a "walk to end" sentinel. Within-contract but semantically stretched; the docstring should explicitly note the helper accepts any value and clamps to last-reachable-line. [src/motions.asm:963]
