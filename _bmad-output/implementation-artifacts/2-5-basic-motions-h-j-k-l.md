...
**Then** the dev pins the policy:
  ...
  **Decision: clear parser state on Esc-to-NORMAL and on every unbound key.**
  ...
  5. **`exline_cancel_core` (src/exline.asm)** — *added by code review (2026-05-15) post-UAT.* Discovered during UAT iteration 1: `exline_cancel_core` inlines its own mode flip to MODE_NORMAL, bypassing `enter_normal_mode`, so the patch on (1) above does NOT cover the `:Esc` round-trip nor any of the `cmd_edit` / `cmd_edit_force` / `cmd_write` / `cmd_write_quit` / `cmd_quit` dirty-refusal completion paths (all of which JP to `exline_cancel_core`). ADD: tail-JP `parser_clear` instead of the trailing `RET`. Net: every NORMAL-mode re-entry from COMMAND mode zeroes parser state. **This is the load-bearing patch for the `5 : Esc h` UAT step 14 verification** — without it, the `5` typed before `:` survives the COMMAND-mode round-trip and `h` moves cursor 5 positions instead of 1.

  **Code-budget impact.** Each `JP parser_clear` is 3 bytes vs the current `RET` (1 byte) — net +2 B per modified handler × ~5 handlers = ~10 B. Negligible against NFR9 pressure. (Updated count: +2 B from the post-UAT `exline_cancel_core` patch.)
...
**Findings from code review (2026-05-15) — three parallel reviewers: Blind Hunter (diff-only adversarial), Edge Case Hunter (diff + project read), Acceptance Auditor (diff + spec).**

Decision-needed → promoted to Patch P5 (user chose "patch — clamp before phantom line"):
- [x] [Review][Decision→P5] **Phantom past-LF cursor on `motion_j` over a trailing-LF file** — `motion_j` from cursor=0 on a 4-byte buffer `"abc\n"` lands cursor at offset 4 (`file_length`), the "phantom" empty line past the trailing LF. The AC4 `eol >= file_length` check is on the LF byte itself (eol=3, file_length=4) so the last-line guard does NOT fire — step proceeds, `next_line_start=4`, `next_line_length=0`, cursor commits to offset 4. Real vi treats trailing LF as terminator, not a phantom new line. Hardware UAT step 11 didn't expose this (likely the test fixture has no trailing LF). **Applied:** P5 patch — added `CALL motion_byte_at_logical ; JR C, .done` after `INC HL ; LD (motions_target_start), HL` in `motion_j` (+5 B); cursor stays put on a file ending in trailing LF. Pinned by new test `motions_j-past-trailing-lf.asm`.

Patches (all applied):
- [x] [Review][Patch] **P1** — AC13 spec amendment: enumerate `exline_cancel_core` as the sixth `parser_clear` patch site. Updated AC13 in this spec to add point 5 documenting the post-UAT addition. Doc-only.
- [x] [Review][Patch] **P2** — `motion_l` `.done` vs `.clamp_undo` trace comments [`src/motions.asm:202-237`]. Inline trace comments at each `JR Z` / `JR C` clarify whether HL is cursor-position vs cursor+1 at that branch; a future simplification can't silently corrupt cursor. Comments only.
- [x] [Review][Patch] **P3** — `motion_k` precondition comment for the "subtract LF byte" DEC [`src/motions.asm:392`]. Added a PRECONDITION block before the `DEC HL` documenting why byte at `current_line_start - 1` is guaranteed to be 0x0A (line-0 guard + `motion_find_line_start` semantics). Comments only.
- [x] [Review][Patch] **P4** — `motion_find_line_start` / `motion_find_line_end` Trashes contract note [`src/motions.asm:534,568`]. Both contracts now clarify "DE trashed transitively via motion_byte_at_logical; this routine itself does not write DE". Comments only.
- [x] [Review][Patch] **P5** — `motion_j` trailing-LF clamp + regression test `motions_j-past-trailing-lf.asm`. +5 B in motion_j; new test pins cursor stays at 0 for buffer `"abc\n"`.

Emergent patch (discovered during patch verification — pre-existing Makefile dependency gap exposed by Story 2.5's diff):
- [x] [Review][Patch] **E1** — added `INCLUDE "../../src/motions.asm"` to 33 test files (3 `dispatch_*.asm` + 30 `exline_*`/`fileio_*`/`init_*`/`parser_*`). Story 2.5 added 4 `motion_*` forward references to `dispatch_normal` in `dispatch.asm` but these tests INCLUDE `dispatch.asm` without `motions.asm`, so they'd fail to build from scratch. The stale `.com` files from before the diff masked the breakage because `test/Makefile`'s rule `cases/%.com: cases/%.asm inc/test_prologue.inc inc/test_epilogue.inc` doesn't track `src/dispatch.asm` as a dependency. Test files patched; underlying Makefile hygiene gap deferred (see new deferred-work entry).

Deferred (pre-existing or scoped-out):
- [x] [Review][Defer] Sticky-column across counted `j`/`k` not preserved — column is recomputed from `cursor_offset` each `.step` iteration, so an intermediate short line ratchets the column down. Per AC's "Note for Story 2.7" the counted-motion semantics are deferred to Story 2.7's verification scope. Edge Case Hunter (E2).
- [x] [Review][Defer] Cursor can rest on a CR byte (rendered as space); editing before CR leaves CR as a phantom byte — the UAT-iteration-2 CR-filter in `render_emit_one_row` patches display only; `motion_h`/`motion_l` don't treat CR as a line boundary. Already in deferred-work.md line 76 (TAB / CR / NUL / high-bit byte handling). Blind Hunter (B10) + Edge Case Hunter (E3).
- [x] [Review][Defer] Test failure diagnostic captures only low byte of `cursor_offset` — `LD A, (cursor_offset) ; LD B, A` reads one byte, masking offsets > 0xFF. Pattern repeats across all motions tests. Improvement for future test debugging; not blocking. Blind Hunter (B8) + Edge Case Hunter (E6).
- [x] [Review][Defer] `motion_byte_at_logical` redundant POP+PUSH pairs [`src/motions.asm:474-505`] — costs ~21 T-states per call for no semantic gain; revisit in Story 2.6 if profiling shows latency. Blind Hunter (B9).
- [x] [Review][Defer] `motion_apply_count` collapses count=0 and count=1 into BC=1 — Story 2.5 doesn't care, but Story 2.11's repeat-operator semantics may need to distinguish "no count typed" from "count=1 typed". Future-trap for Story 2.11. Blind Hunter (B12).
- [x] [Review][Defer] `motions_col` / `motions_target_start` scratch cells retain mid-step values on `.done` early-exit — not read across motion invocations today, but a future feature (e.g. `gj` display-column-preserving variant) would inherit garbage. Add INVARIANT comment or zero them in parser_clear. Blind Hunter (B15).
- [x] [Review][Defer] Missing test: `motion_j` onto an empty middle line (cursor lands on the LF byte) — AC4 documents this edge case but no test pins it. Add `motions_j-onto-empty-line.asm` covering buffer `"a\n\nb"` cursor=0 expecting cursor=2. Edge Case Hunter (E7).
- [x] [Review][Defer] `motion_h` / `motion_l` / `motion_find_line_start` don't check CF after `motion_byte_at_logical` — invariant-protected today (cursor <= file_length); defensive improvement that would make the helper contract self-consistent (helper signals CF for a reason). Blind Hunter (B3).
- [x] [Review][Defer] `enter_*_mode` Trashes attribution misleading — combined attribution `(via status_set_message + parser_clear)` implies parser_clear shares blame for BC/DE; per verified `parser.asm:172`, parser_clear only trashes A/HL/F. Cosmetic doc clarity. Blind Hunter (B13).
- [x] [Review][Defer] AC10 spec amendment for `render.asm` CR-filter scope creep — the UAT-iteration-2 CR-filter in `render_emit_one_row` is a render change inside Story 2.5 even though AC10 said "no render modifications". Documented in Change Log row 4 but AC10 spec text remains literal. Acceptance Auditor's secondary finding.

Dismissed as noise / verified / spec-authorised (8): BC-preservation invariant (Blind B1 — already in preamble); parser_clear Trashes contract inconsistency (Blind B6 — verified consistent against parser.asm:172); exline_cancel callpath coverage gap (Blind B7 — verified: all 10 routes through exline_cancel_core); ex_buffer length-byte test layout (Blind B14 — verified: state.inc:100, ex_buffer is the length byte); count overflow at 65536 (Edge E4 — spec-authorised, AC6 "Note on count overflow"); g+motion silently drops g (Edge E5 — spec-authorised, AC7); AC3/4/5 PARTIAL (Auditor — documented divergences in "Decisions made during dev"); AC14/AC15 PARTIAL (Auditor — build-only and per-spec deferral).
...
| 2026-05-15 | **Code review applied 5 patches + 1 emergent batch fix.** Three parallel reviewers (Blind Hunter / Edge Case Hunter / Acceptance Auditor) surfaced 1 decision-needed, 4 patches, 10 deferred, 8 dismissed. (P5, CODE-CHANGE) `motion_j` trailing-LF clamp fix: on a file ending in `0x0A` (e.g. `"abc\n"`), the literal AC4 algorithm advanced cursor to `file_length` (the phantom empty line past the LF) because the `eol >= file_length` guard fired on the LF position not on `next_line_start`; +5 B `CALL motion_byte_at_logical ; JR C, .done` inserted between `LD (motions_target_start), HL` and the second `motion_find_line_end` call. Hardware UAT step 11 didn't expose this — likely the UAT fixture lacks a trailing LF. New regression test `motions_j-past-trailing-lf.asm` pins cursor stays at 0 for buffer `"abc\n"`. (P1, DOC) AC13 spec text amended to enumerate `exline_cancel_core` as the sixth `parser_clear` patch site (the original AC13 listed 5; UAT iteration 1 reactively added the 6th — Auditor's "most important finding"). (P2-P4, COMMENTS) Inline trace comments on `motion_l`'s `.done`/`.clamp_undo` split, precondition block on `motion_k`'s "subtract LF byte" DEC, and Trashes contract clarification for `motion_find_line_start`/`motion_find_line_end` (DE trashed transitively via callee, not locally). Zero byte impact. (E1, EMERGENT) Discovered a pre-existing test/Makefile dependency gap: 33 test files INCLUDE `src/dispatch.asm` without `src/motions.asm`, so Story 2.5's 4 new `motion_*` forward references in `dispatch_normal` broke their build. Stale `.com` files masked the breakage because the Makefile's `cases/%.com` rule doesn't track `src/*.asm` as a dependency. Patched: `INCLUDE "../../src/motions.asm"` inserted after `INCLUDE "../../src/parser.asm"` in 3 `dispatch_*.asm` + 30 other tests. Underlying Makefile hygiene gap deferred. Post-patch SHA `48ba49b2889d32c9d2014d8400ce227131be40e79b2ed839a0dbac4b7ec4938a`, byte-identical second rebuild (NFR18). Size **4082 B** / ~132% of original NFR9 / **14 B** under proposed 4096 B amended ceiling (+5 B vs pre-review 4077 B; all increment is P5). 63 pass / 1 deliberate fail (+1 new motions_j-past-trailing-lf test). 10 items deferred to `deferred-work.md` (sticky-column across counted j/k; cursor-on-CR edit-time invariant; single-byte test diagnostic; motion_byte_at_logical perf POP/PUSH; count=0/1 collision; motions_col scratch-cell reset; motion_j-onto-empty-line test gap; CF-check defensive improvement; enter_*_mode Trashes attribution; AC10 render scope-creep doc; **test/Makefile dependency hygiene gap** — added by this review). ~8 dismissed as noise / verified-safe / spec-authorised (verified parser_clear trashes A/HL/F per parser.asm:172; verified all 10 exline_cancel_core call sites route through the new tail-JP; verified ex_buffer is the length byte per state.inc:100). Acceptance Auditor: 9 ACs MET, 6 PARTIAL (all documented divergences), 1 PARTIAL+1 NOT-VERIFIABLE-IN-DIFF for AC14/AC12; zero UNDOCUMENTED divergences. Final build SHA `48ba49b2…ec4938a`. |
