...
| 6 | Press `$a` to append at EOF | Cursor lands at offset 13 (last printable byte of row 2, the rightmost 'x'); status `-- insert --`. Per [[feedback_uat_trace_cursor]]: `$` then `a` appends after $ position |
