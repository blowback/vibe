...
### Review Findings (post-hoc bmad-code-review 2026-05-19)

Sources: Blind Hunter (diff-only) + Edge Case Hunter (diff + repo) + Acceptance Auditor (diff vs spec). 17 raw findings → 1 decision-needed, 3 patch, 7 defer, 6 dismissed.

- [x] [Review][Patch] (was Decision D1) AC1 `file_length=0` short-circuit regression-pin — **RESOLVED 2026-05-19:** added `test/cases/visual_tilde-empty-buffer.asm` (sentinel 0x97). VIS_CHAR arm fires `~` on empty buffer with cursor=anchor=0 (drives BC=1 into `gapbuf_case_toggle_range`); pre-seeds `yank_buffer[0]` with sentinel 0xA5 and asserts it survives — without the AC1 guard this byte is XOR-toggled (Story 3.8 caller-bound finding 1). Also pins mode→NORMAL, cursor=0, file_length=0, buffer_dirty=0, undo_kind=0 (Q3 Option A preserve). Test count 267 → 268. NFR18 SHA `d791eea13ff782aa4818aad7f87a3667992c89402a103a50211b4091ca109543` reverified post-build (binary unchanged — test-only addition).
- [x] [Review][Patch] T3 test header comment math contradicts the test's own assertions [`test/cases/visual_line_toggle-last-line-no-lf.asm`] — **RESOLVED 2026-05-19:** comment claimed `motion_find_line_end(4)→HL=6` but the function contract returns `HL = file_length` (= 7) on the no-LF path; comment also misread `range_end = HL = 6 (not HL+1)` when the actual computation is `range_end = HL = 7`, BC = 7 − 4 = 3. Rewrote the comment to spell out the file_length contract and the CF=1 skip-INC behavior. Test code unchanged. (Source: Blind Hunter F2.)
- [x] [Review][Dismiss] AC1 guard comment "SR3 math" reference — **DISMISSED as false positive 2026-05-19:** SR3 IS a documented tag in `motions.asm:551` (`--- Internal helper: motion_byte_at_logical (SR3 byte-read) ---`) and `:562` (`SR3 math:` block). The Blind Hunter missed the upstream definition. Comment in `src/gapbuf.asm:322` is correct as-is. (Source: Blind Hunter F4 — invalid.)
- [x] [Review][Patch] `motions.asm` AR23 per-function callout doesn't cross-reference `_visual_op_block_cursor_clamp` as the non-bracketing exception — **RESOLVED 2026-05-19:** added a 4-line "Exception:" clause to the `*** DE-TRASH IS LOAD-BEARING ***` block at `src/motions.asm:577-584` pointing to `_visual_op_block_cursor_clamp` as the in-tree precedent for safely omitting PUSH/POP DE bracketing when the caller body never reads DE between the CALL and any subsequent observer. Comment-only; NFR18 byte-identical preserved. (Source: Edge Case Hunter F5.)
- [x] [Review][Defer] AC2 reproducing-fixture test file `visual_block-clamp-loop-walks-back-multibyte.asm` named in spec is not in the diff — deferred, AC2 functionally MET via the helper; the multi-step walk-back regression-pin promised in Sub 6.4 was skipped.
- [x] [Review][Defer] AC2 spec narrative arithmetic is internally inconsistent (`anchor offset 0 col 0 + cursor offset 6 col 4` derivation trails off; later cross-check uses different chain `7→6→5→4→3→3→2`) — deferred, spec text drift only; implementation is correct.
- [x] [Review][Defer] AC1 narrative says `~10 B` in AC body but Completion Notes record `+17 B` — deferred, story doc drift; the Debug Log already documents *why* (gap_end-only check false-positives at cursor-at-EOF, switched to explicit `file_length` compute). AC body should be amended on next pass.
- [x] [Review][Defer] `sprint-status.yaml` `last_updated` audit trail skips intermediate transitions (ready-for-dev → in-progress → review → done) — deferred, process-hygiene only; the state itself is correct.
- [x] [Review][Defer] `_visual_op_block_cursor_clamp` "Preserves BC" docstring claim transitively depends on `motion_byte_at_logical` preserving BC — deferred, contract is documented (motions.asm BC-preservation invariant block) but not statically verifiable; relies on author discipline. If a future change loosens `motion_byte_at_logical`'s BC contract, both BLOCK clamp sites silently break.
- [x] [Review][Defer] `_visual_op_block_cursor_clamp` can land cursor on an LF byte at offset 0 (e.g. buffer `"\n\n\n"` or any starts-with-LF buffer) — deferred, the HL=0 underflow guard fires before the LF check, so cursor lands ON the LF. Behavior diverges from the comment's "walks back while past-EOF or LF" intent. Real but low priority; depends on how VIBE renders cursor-at-LF.
- [x] [Review][Defer] `test/cases/gapbuf_case_toggle_range-crosses-gap.asm` hand-constructs `gap_start=base+2, cursor_offset=0` (violates gap-tracks-cursor by design) — deferred, the construction works today because `gapbuf_move_gap` happens to copy only the after-gap region; a future change to gap-relocation could non-deterministically fail this test on uninitialized HRAM.

**Dismissed:** AC1 implementation form deviation (justified in Debug Log); T9 inverted semantics (documented per Task 4.10 BLOCK Q1 Option A); AC6 NFR9 (MET); scope creep (MET — clean); O(N) clamp walk (correctness fine, YAGNI); speculative post-delete coupling (no actual bug found).
