...
| 2026-05-16 | Code review pass complete: 7 patches applied, 3 deferred, 12 dismissed | 1 P1 (Edge): `motions_count-cleared-post-dispatch.asm` Subtest 3 reworked (cursor=6/count=3 → cursor=3 → cursor=2 by 1) — original fixture collided moved-by-1 with moved-by-3 at cursor=0; AC6 invariant now genuinely pinned. 2 P2: (a) `deferred-work.md:212` amended to reflect post-Option-B `motions_col` write-once-at-entry lifetime; (b) `motions_leading-zero-still-motion-0.asm` strengthened with a `pending_motion_prefix='g'` pre-seed + Subtest 3 (sentinel 0x82) — vacuous count assertion no longer the only parser_clear-chain check. 4 P3 doc fixes: `motions_100j-clamps-at-eof.asm` "4 lines" → "5 lines"; `motions_count-cleared-post-dispatch.asm` header rewritten; `motion_j` header step 7 cross-reference annotation restored; Completion Notes "All 16 ACs" → "All 15 ACs". Deferred: counted-k sticky-column test, empty-intermediate-line sticky-column test, low-byte-only diagnostic capture pattern. |
| 2026-05-16 | Post-review build invariants verified                                  | Build SHA `e72d102815342b2a5333d4f31314cd1a032998d90515ceaf5bc597429a9b1d62` byte-identical across two `make clean && make all` (NFR18). Size unchanged at 4380 B / 85.5% / 740 B headroom — patches were comment-only in `motions.asm` and test/admin-only elsewhere. 92 pass / 1 deliberate fail (harness_fail). The three patched tests (`motions_count-cleared-post-dispatch` / `motions_leading-zero-still-motion-0` / `motions_100j-clamps-at-eof`) all green after rework. |
| 2026-05-16 | Story status: review → done                                            | All ACs MET (AC13 user-attested); review patches applied; defers logged to deferred-work.md. |

### Review Findings

Code review 2026-05-16 (Blind Hunter + Edge Case Hunter + Acceptance Auditor, 22 raw findings → 7 patches / 3 defers / 12 dismissed). Acceptance Auditor: 14 ACs MET / 0 PARTIAL / 0 NOT MET / 1 NOT VERIFIABLE-IN-DIFF (AC13 hardware UAT, user-attested).

- [x] [Review][Patch] **P1 (Edge): `motions_count-cleared-post-dispatch.asm` Subtest 3 cannot distinguish moved-by-1 from moved-by-5.** AC6's load-bearing invariant ("unprefixed motion after a counted motion moves by 1") is invisible to the assertion. Current fixture: cursor=6, count=5, buffer `"abcdefg"`. After Subtest 1 cursor=1, count cleared. Subtest 3 from cursor=1 with count=0 lands at cursor=0 (moved by 1, BOF clamp). BUT if a regression left count_accumulator stuck at 5, motion_h from cursor=1 with count=5 ALSO lands at cursor=0 (1→0, then HL=0 fires the .done early-exit). Both outcomes collide at cursor=0. Fix: rework Subtest 1 to cursor=6, count=3 → cursor=3; Subtest 3 from cursor=3 with count=0 → cursor=2 (moved by 1) — distinct from cursor=0 (would-be by 3 if regression). [`test/cases/motions_count-cleared-post-dispatch.asm:45-89`]
- [x] [Review][Patch] **P2 (Blind): Stale `motions_col` cell-lifetime claim in deferred-work.md line 212.** Pre-Option-B entry says `motions_col` / `motions_target_start` "cells are written by motion_j / motion_k step bodies only." After Option B, `motions_col` is written ONCE at entry preamble (before `.step:`), NOT inside step bodies. `motions_target_start` is still written per-step (no change). Fix: amend line 212 to reflect post-2.7 lifetime — `motions_col` written at motion_j/k entry; `motions_target_start` written in step body. The "stack-manipulation through 4 nested helpers" rationale in the same entry also no longer applies for motions_col (one helper call now, not four). [`_bmad-output/implementation-artifacts/deferred-work.md:212`]
- [x] [Review][Patch] **P2 (Edge): `motions_leading-zero-still-motion-0.asm` count assertion is vacuous on parser_clear chain.** AC8's load-bearing invariant is "leading '0' → JP motion_0 → tail-JP parser_clear." Test pre-zeros count, asserts count==0 post-call — but count was already 0 entering, so the assertion catches only an active-write-nonzero regression (essentially impossible). Cursor assertion catches "motion_0 didn't run." Neither catches "motion_0 ran but tail-JP parser_clear didn't fire." Fix: pre-seed `pending_motion_prefix` to a sentinel (e.g. 0x67 = 'g'), assert it's 0 after — genuinely pins motion_0's tail-JP parser_clear. [`test/cases/motions_leading-zero-still-motion-0.asm:30-65`]
- [x] [Review][Patch] **P3 (Blind): `motions_100j-clamps-at-eof.asm` header comment "walks 4 lines down" is off by one — it's 5 hops** (line 5 → line 10 via 5→6→7→8→9→10). Arithmetic in the assertion (cursor=36 = 4*9 = line 10 start) is correct; only the doc string is wrong. [`test/cases/motions_100j-clamps-at-eof.asm` header]
- [x] [Review][Patch] **P3 (Blind): `motions_count-cleared-post-dispatch.asm` Subtest-1 header comment is misleading.** Comment claims "walks 5 steps left within single line" but separately mentions "BH2 BOF-region clamp from the .step's intra-line LF check" — the fixture `"abcdefg"` has no LF; Subtest 1 doesn't exercise any clamp. Fix: drop the BH2-clamp phrasing; it's a vanilla count-walk. (Patch this alongside the P1 rework so the comments match the new fixture.) [`test/cases/motions_count-cleared-post-dispatch.asm` header]
- [x] [Review][Patch] **P3 (Blind): `motion_j` header step-1 lost the `[save in motions_col]` cross-reference annotation that the old per-step algorithm carried.** The new STICKY COLUMN INVARIANT block above the algorithm covers it, but the per-step algorithm now reads `new_col = min(motions_col, clamp_col)` without an in-block "(motions_col = entry col, captured pre-loop)" annotation. Tiny doc tweak for readability. [`src/motions.asm` motion_j header docstring]
- [x] [Review][Patch] **P3 (Auditor): Completion Notes says "All 16 ACs" but the spec defines AC1..AC15** (no AC16). Tiny miscount. [`_bmad-output/implementation-artifacts/2-7-counted-motions.md:468`]
- [x] [Review][Defer] **D1 (Edge): No test pins counted-k sticky-column** — `motions_count-j-sticky-column.asm` only covers motion_j; the motion_k hoist + at-line-0 refactor (LD A,D/OR E → LD A,H/OR L) is exercised only by single-step Story-2.5 tests. A future regression in counted-k specifically (e.g. wrong register in the at-line-0 check) could escape. Defer: mirror j's test as `motions_count-k-sticky-column.asm` in a follow-up pass. [`test/cases/`]
- [x] [Review][Defer] **D2 (Edge): No test covers the empty-intermediate-line variant of sticky-column** — `"hello\n\nworld"` cursor=4 count=2 should land at cursor=11 (col 4 of "world"). Strongest variant of Option B because step 1's clamp_col=0 forces cursor onto the LF byte itself, and step 2 must recover the original col from motions_col rather than re-derive from cursor-on-LF. Current test (`"hello\nab\nworld"`) covers the length-2 intermediate case; length-0 is the harder corner. Defer: add `motions_count-j-sticky-column-empty-line.asm`. [`test/cases/`]
- [x] [Review][Defer] **D3 (Blind + Edge): Low-byte-only diagnostic cursor capture across all `motions_*` and `parser_*` tests** — `LD A, (cursor_offset) ; LD B, A` discards H. Already tracked in deferred-work.md line 234 (existing Story 2.5/2.6 entry); 11 new Story 2.7 tests inherit the pattern. Defer with the existing entry. [`test/cases/motions_*.asm`, `test/cases/parser_*.asm`]

**Dismissed as noise (12, summarized):** "Symmetric with motion_j" header refers to semantic operation not implementation pattern (verified — Completion Notes capture the implementation divergence); `motion_k` DE-state at `LD (motions_target_start), HL` correctly recovered by `POP DE ; EX DE, HL` immediately after (verified by reading full motion_k); per-step `motion_find_line_start` BC-preservation documented at the helper itself (line 596) — call-site annotation not required at every site; `motions_esc-clears-count.asm` `LD HL, 3 ; LD (count_accumulator), HL` covers both bytes; mixed 1-indexed lines / 0-indexed cols is vi convention; two `last_updated:` keys in sprint-status.yaml follow established append-style convention; `Sub 3.6` reference in test comment is documented in spec Change Log; defensive pre-zero of `pending_operator`/`pending_motion_prefix` in tests is intentional hygiene; spec internal "11 vs 12 test files" inconsistency is harmless (dev delivered 11 per Sub 6.5 / Sub 7.3); Sub 3.6/4.6 spec arithmetic bug already caught and pinned correctly by dev (test asserts cursor=13).
